...
//"http://nexoya.cl:8080/apiSimple/users/list/small";
//"http://private-anon-6dbb3df949-apisimple1.apiary-mock.com/users/list/small";
	$ch = curl_init($url);
	...
	$display.='<select size="35" style="width:270px" data-placeholder="Rut - Nombre"  class="chosen" id= "'.$this->id.'"  name="'.$this->nombre.'" ' . ($modo == 'visualizacion' ? 'readonly' : '') . ' data-modo="'.$modo.'" >';
	$display.='<option value="null"> </option>';
	ChromePhp::log($array_json);	
	if($array_json) for ($cont = 0; $cont < count($array_json); $cont++) {
		if($dato){
		$display.='<option value="' .$array_json[$cont]->lastName." ".$array_json[$cont]->name.'-'.$array_json[$cont]->rut.'-'.$array_json[$cont]->location .'-'.$array_json[$cont]->costCenter.'-'.$array_json[$cont]->service.'"'.($array_json[$cont]->lastName." ".$array_json[$cont]->name.'-'.$array_json[$cont]->rut.'-'.$array_json[$cont]->location == $dato->valor ? 'selected' : ' ') .'>'.$array_json[$cont]->lastName.' '.$array_json[$cont]->name.' - '.$array_json[$cont]->rut.'</option>';
		}else{
		$display.='<option value="' .$array_json[$cont]->lastName." ".$array_json[$cont]->name.'-'.$array_json[$cont]->rut.'-'.$array_json[$cont]->location .'-'.$array_json[$cont]->costCenter.'-'.$array_json[$cont]->service. '"'.($array_json[$cont]->lastName." ".$array_json[$cont]->name.'-'.$array_json[$cont]->rut.'-'.$array_json[$cont]->location == $valor_default ? 'selected' : ' ') .'>'.$array_json[$cont]->lastName.' '.$array_json[$cont]->name.' - '.$array_json[$cont]->rut.'</option>';
